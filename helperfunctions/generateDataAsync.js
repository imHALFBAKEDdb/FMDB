...
var fs = require("fs");
const writer = fs.createWriteStream("dataASYNCmongo.tsv");
...
var randomGenre = () => {
  let genres = ["horror", "romance", "comedy", "documentary", "action", "thriller", "drama", "western", "animated", "science fiction", "musical theatre", "indie", "mystery", "fantasy", "martial arts", "anime", "superhero"];
...
var header = "Title\tYear\timdbID\tType\tPoster\treview_average\treview_count\textra_image\tdirector\twriter\trelease_date\tmovie_length\tactor_info\tmovie_genre\treview_date\treviewer\treview_title\treviews_number\treview_viewers\treview\tmovie_description\tmovie_id\n"
var t0 = thetimeisnow();
writer.write(header)
...
var generate1M = (j) => {
  ...
  for(let i = 0; i < 100000; i++) {
    ...
    let str = `${faker.commerce.productName()}\t${faker.date.past()}\t${randomYear()}\ttt${faker.random.number()}\t${faker.image.people()}\t${Math.floor(Math.random() * 5 + 1)}\t${faker.random.number()}\t${faker.image.animals()}\t${faker.name.findName()}\t${faker.name.findName()}\t${faker.date.past()}\t${randomMovieLength()}\t${randomActorInfo()}\t${randomGenre()}\t${faker.date.past()}\t${faker.name.findName()}\t${faker.random.word()}\t${faker.random.number()}\t${faker.random.number()}\t${faker.lorem.paragraph()}\t${faker.lorem.words()}\t${index()}\n`;
    //let str = `${faker.commerce.productName()}\t${faker.date.past()}\t${randomYear()}\ttt${faker.random.number()}\t${faker.image.people()}\t${Math.floor(Math.random() * 5 + 1)}\t${faker.random.number()}\t${faker.image.animals()}\t${faker.name.findName()}\t${faker.name.findName()}\t${faker.date.past()}\t${randomMovieLength()}\t${randomActorInfo()}\t${randomGenre()}\t${faker.date.past()}\t${faker.name.findName()}\t${faker.random.word()}\t${faker.random.number()}\t${faker.random.number()}\t${faker.lorem.paragraph()}\t${faker.lorem.words()}\t${randomYear()}\t${newDate}\t${newDate}\n`;
    writer.write(str);
  }
